...
import { AlertItem, EnvironmentalSnapshot, HotspotDetection, LivePredictionResponse, MetricSource, RegionRisk, SentinelSceneStatus, WeatherSnapshot } from "../types";
...
async function loadLiveAlerts(): Promise<{ alerts: AlertItem[]; source: MetricSource }> {
  const responses = await Promise.allSettled(regions.map((region) => api.get<LiveAlertEvaluationResponse>(`/alerts/evaluate/${region.id}`)));
  const evaluatedAlerts: AlertItem[] = [];
  const successfulEvaluations: LiveAlertEvaluationResponse[] = [];
  const unavailableEvaluations: LiveAlertEvaluationResponse[] = [];

  for (const result of responses) {
    if (result.status !== "fulfilled") continue;
    const evaluation = result.value.data;
    if (evaluation.status === "ok") successfulEvaluations.push(evaluation);
    if (evaluation.status === "unavailable") unavailableEvaluations.push(evaluation);
    evaluatedAlerts.push(...evaluation.alerts.map(alertFromApi));
  }
  ...
  if (evaluatedAlerts.length > 0) {
    return {
      ...
      source: {
        ...
        sourceType: "live_prediction_alert_evaluation",
        dataStatus: "LIVE",
        message: "Live alerts generated by the existing alert engine from current prediction output."
      }
    };
  }
  ...
  if (successfulEvaluations.length > 0) {
    return {
      ...
      source: {
        ...
        sourceType: "live_prediction_alert_evaluation",
        dataStatus: "LIVE",
        message: "Live prediction evaluated by the alert engine; no active live alerts."
      }
    };
  }
  ...
  return {
    alerts: [],
    source: {
      ...unavailableAlertSource,
      message: unavailableEvaluations[0]?.message ?? unavailableAlertSource.message
    }
  };
}
...
export function useEnvironmentalData() {
  return useQuery({
    ...
    queryFn: async () => {
      ...
      try {
        const [liveWeather, liveHotspots, liveSentinel, livePredictions, liveAlerts] = await Promise.all([loadLiveWeather(), loadLiveHotspots(), loadLiveSentinelScenes(), loadLivePredictions(), loadLiveAlerts()]);
